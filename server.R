...
output$selected_var <- renderPrint({ 
  print(h2o.performance(aml(), newdata =split()[[2]]))
})
#########################Predictions##############################
test<-reactive({
  validate(
...
observe({
  row=seq(1, h2o.nrow(split()[[2]]), by=1)
  updateSelectizeInput(session,'obs', choices=row,selected=row[1])
...
output$lime <- renderPlot({
  ...
  explainer <- lime::lime(as.data.frame(split()[[1]][1:100,2:11]), aml(), bin_continuous = FALSE)  
  
  #Run explain() on explainer
  v=as.data.frame(split()[[2]][,2:11])
  explanation <- lime::explain(v[input$obs,], explainer, n_labels = 1, n_features = 6, kernel_width = 0.5)
